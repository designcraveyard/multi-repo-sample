...
|----------|--------|--------|--------------|
| Web | `multi-repo-nextjs/app/components/Native/AppAlertPopup.tsx` | Present | `isPresented: boolean`, `onClose: () => void`, `title: string`, `message?: string`, `buttons?: AlertButton[]`, `className?: string` |
| iOS | `multi-repo-ios/multi-repo-ios/Components/Native/AppAlertPopup.swift` | Present | See source file for the public API. |
| Android | `multi-repo-android/app/src/main/java/com/abhishekverma/multirepo/ui/native/AppAlertPopup.kt` | Present | `isPresented: Boolean`, `onDismiss: () -> Unit`, `title: String`, `message: String? = null`, `buttons: List<AlertButton> = listOf(AlertButton("Cancel", role = AlertButtonRole.Cancel) {})` |
