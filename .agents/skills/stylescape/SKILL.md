...
4. **Pre-flight checks:**
   ...
   - Check `node_modules` in `.Codex/plugins/asset-gen/`:
     - Missing → run `cd .Codex/plugins/asset-gen && npm install`
...
Invoke via Bash:
```bash
node .Codex/plugins/asset-gen/generate.js --spec /tmp/stylescape-gen-spec.json --env .env.local
```
...
The stylescape CSS lives at `.Codex/skills/stylescape/stylescape.css`.
