...
For each `$TEMPLATE_DIR/.Codex/skills/*/SKILL.md`:
...
Same approach for `$TEMPLATE_DIR/.Codex/agents/*.md`.

### Category C: Plugins (Low Risk)

For each plugin directory in `$TEMPLATE_DIR/.Codex/plugins/`:
...
Compare `$TEMPLATE_DIR/.Codex/settings.json` vs `.Codex/settings.json`.
...
**Never sync:** `AGENTS.md`, `tracker.md`, `chatkit.config.json`, `.mcp.json`,
...
```bash
cp .Codex/settings.json .Codex/settings.json.backup-$(date +%Y%m%d) 2>/dev/null
```
...
4. For NEW skills, create the directory: `mkdir -p .Codex/skills/<name>/`

### Step 5.3: Settings.json Hook Merge (Additive Only)

**Never remove existing hooks.** Only add or replace.

1. Read child's `.Codex/settings.json`
2. Read template's `.Codex/settings.json`
...
7. Write merged JSON back, validate with:
   ```bash
   python3 -c "import json; json.load(open('.Codex/settings.json'))"
   ```
...
grep -rn "multi-repo-nextjs\|multi-repo-ios\|multi-repo-android\|multi-repo-sample\|multi_repo_ios\|MultiRepo" \
  .Codex/skills/ .Codex/agents/ .Codex/plugins/ .Codex/settings.json 2>/dev/null
```
...
Backup: .Codex/settings.json.backup-YYYYMMDD
```

### Step 6.3: Suggest Follow-ups

- If hooks were added: "Try editing a component file to confirm the new hooks fire."
- If new skills were added: "New skills available: /skill-name. See AGENTS.md."
- Always: "AGENTS.md was not synced (project-specific). Manually port any new
  conventions you need from the template's AGENTS.md."
...
5. **Never sync `AGENTS.md`** — project-specific documentation
