...
The wireframe CSS lives as a bundled skill resource at `.Codex/skills/wireframe/wireframe.css`.

On first run, copy it to `docs/wireframes/_wireframe.css` if not already present:
```bash
cp .Codex/skills/wireframe/wireframe.css docs/wireframes/_wireframe.css 2>/dev/null || true
```
...
See `figma-cli/AGENTS.md` for full JSX render syntax.
