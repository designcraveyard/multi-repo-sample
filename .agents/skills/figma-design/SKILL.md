...
| Component ID cache | `.Codex/figma-component-map.json` |
...
**Trigger:** Run this phase if `.Codex/figma-component-map.json` does NOT exist OR `--refresh-map` was passed.
...
4. Write `.Codex/figma-component-map.json`:
   ```json
   {
     "AppButton": "375:100",
     "AppListItem": "375:102",
...
5. **Component ID cache** — read `.Codex/figma-component-map.json` (if it exists).
...
| Flag | Behaviour |
|------|-----------|
| `--refresh-map` | Re-run Phase 0, overwrite `.Codex/figma-component-map.json` |
