...
- Plugin dependencies installed: `cd .Codex/plugins/asset-gen && npm install`
...
- Use Codex vision to extract: color tendencies, rendering style, composition patterns, mood, subject matter
...
ls .env.local
node --version
ls .Codex/plugins/asset-gen/node_modules/.bin/node 2>/dev/null || echo "not installed"
```

If dependencies not installed:
```bash
cd .Codex/plugins/asset-gen && npm install
```
...
```bash
node .Codex/plugins/asset-gen/generate.js --spec /tmp/asset-gen-spec.json --env .env.local
```
