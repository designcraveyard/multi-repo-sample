...
→ Extract signals from new reference via Codex vision, incorporate into prompt, then generate (image-to-image if appropriate)
...
- Analyze the shared image with Codex vision
...
```bash
node .Codex/plugins/asset-gen/iterate.js --spec /tmp/asset-iterate-spec.json --env .env.local
```
