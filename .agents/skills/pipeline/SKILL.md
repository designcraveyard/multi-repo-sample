...
```
User: /pipeline
Codex: [Creates pipeline.json, asks about Figma, shows overview, begins with Phase 2]
```

### Resume After Break
```
User: /pipeline resume
Codex: [Reads pipeline.json, shows progress, resumes from design_wireframes]
```

### Code-First Mode
```
User: /pipeline --skip-figma
Codex: [Sets flag, auto-skips phase 12, adapts phases 7/9 to skip Figma CLI]
```

### Skip to Specific Phase
```
User: /pipeline --from schema
Codex: [Validates all prerequisites for schema phase, proceeds if valid]
```
