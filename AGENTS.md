...
**MCP Server Builder** (`.Codex/plugins/mcp-server-builder/`): `/new-mcp-server` (8-phase scaffold wizard). Includes `mcp-server-reviewer` agent. Advisory hooks: `console-log-guard`, `auth-middleware-reminder`, `mcp-json-reminder`.
...
The only retained AI runtime is lightweight transform/transcribe. Web keeps `/api/ai/*`, `lib/openai/`, `useTransformStream`, and the MarkdownEditor AI toolbar. iOS keeps `OpenAI/TransformService.swift`, `OpenAI/TranscribeService.swift`, `OpenAI/OpenAITypes.swift`, `OpenAI/Configs/MarkdownTransformConfig.swift`, `Audio/AppAudioRecorder.swift`, `AIDemoView`, and the MarkdownEditor AI toolbar. Supabase keeps JWT-protected Edge Functions `ai-transform` and `ai-transcribe`. The OpenAI Agents SDK chat/admin/RAG stack has been removed.
